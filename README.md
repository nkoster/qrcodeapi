
This is a too simple example of an API that receives a string and returns a JSON in the following format:

```
{
  "qrqode": "<base64>"
}
 ```

Example:

```
curl localhost:10000/example.com
{"qrcode":""}
```
This returns a JSON object, that contains a base64 encoded PNG, that contains a QRcode, that contains a string "https://example.com", in less than 30 lines of code.

Dependencies:
```
apt install qrencode
```
